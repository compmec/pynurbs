...
@pytest.mark.dependency(
    depends=[
        ...
        "tests/core/test_piecewise.py::test_all",
    ],
    scope="session",
)
def test_begin():
...
class TestBezier:
    ...
    def test_piecewise_polynomial(self):
        ...
        # Degree 0
        knotvector = ImmutableKnotVector([0, 1])
        bezier = ImmutableSplineBasis(knotvector)
        assert bezier.degree == 0
        assert bezier.npts == 1
        assert bezier.knots == (0, 1)
        assert bezier[0] == PiecewisePolynomial([1 + 0 * x], [0, 1])
        ...
        assert bezier.npts == 2
        assert bezier.knots == (0, 1)
        assert bezier[0] == PiecewisePolynomial([1 - x], [0, 1])
        assert bezier[1] == PiecewisePolynomial([x], [0, 1])
        ...
        assert bezier.npts == 3
        assert bezier.knots == (0, 1)
        assert bezier[0] == PiecewisePolynomial([(1 - x) ** 2], [0, 1])
    ...
    @pytest.mark.dependency(
        depends=[
            ...
            "TestBezier::test_sum_equal_to_1",
            "TestBezier::test_single_values",
            "TestBezier::test_piecewise_polynomial",
        ]
    )
    def test_all(self):
        pass
